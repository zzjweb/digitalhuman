        ...
        if self.config.rollout.name == 'hf':
            ...
            if self.config.rollout.use_fire_sampling:
                ...
                if self.config.thinking:
                    from verl.workers.rollout.vllm_rollout import vLLMRollout, vllm_mode, vLLMMultiTurnViaChatRollout_think
                else:
                    from verl.workers.rollout.vllm_rollout import vLLMRollout, vllm_mode, vLLMMultiTurnViaChatRollout
            ...
            #                                    model_hf_config=self.actor_model_config)
                ...
                if self.config.thinking:
                    from verl.workers.rollout.vllm_rollout import vLLMMultiTurnViaChatRollout_think
                else:
                    from verl.workers.rollout.vllm_rollout import vLLMMultiTurnViaChatRollout

                if vllm_mode == 'customized':
                    if self.config.thinking:
                        rollout = vLLMMultiTurnViaChatRollout_thinkvLLMMultiTurnViaChatRollout(actor_module=self.actor_module_fsdp,
                                        config=self.config.rollout,
                                        tokenizer=self.tokenizer,
                                        model_hf_config=self.actor_model_config)
                    else:
                    ...
                    if self.config.thinking:
                        rollout = vLLMMultiTurnViaChatRollout_think(model_path=local_path,
                                        config=self.config.rollout,
                                        tokenizer=self.tokenizer,
                                        model_hf_config=self.actor_model_config,
                                        device_mesh=rollout_device_mesh)
                    else:
                        rollout = vLLMMultiTurnViaChatRollout(model_path=local_path,
                                        config=self.config.rollout,
                                        tokenizer=self.tokenizer,
                                        model_hf_config=self.actor_model_config,
                                        device_mesh=rollout_device_mesh)
