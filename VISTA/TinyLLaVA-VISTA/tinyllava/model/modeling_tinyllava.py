...
class TinyLlavaForConditionalGeneration(TinyLlavaPreTrainedModel):
    ...
    def forward(
        ...
        return self.language_model.forward(
            ...
            label_embeddings=labels_embeddings,
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        embedding_size = self.language_model.get_input_embeddings().weight.shape[1]
        cur_image_idx = 0
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                cur_new_labels_embeddings = torch.full(
                    (cur_input_embeds.shape[0], embedding_size),
                    0,
        ...
        for i, (cur_new_embed, cur_new_labels) in enumerate(zip(new_input_embeds, new_labels)):
            ...
            if getattr(self.config, 'tokenizer_padding_side', 'right') == "left":
                    ...
                    if new_labels_embeddings_padded is not None:
                        if i < len(new_labels_embeddings):
                            try:
                                ...
                                print("Error in setting new_labels_embeddings_padded", e)
                                pass
                ...
                if cur_len > 0:
                    new_labels_padded[i, :cur_len] = cur_new_labels
                    if new_labels_embeddings_padded is not None:
                        # print("i", i, len(new_labels_embeddings))
                        # print("i", i, len(new_labels_embeddings_padded))
                        if i < len(new_labels_embeddings):
                            try:
                                new_labels_embeddings_padded[i, :cur_len] = (
                                    new_labels_embeddings[i].float()
        ...
        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels, new_labels_embeddings, new_text_embeds if new_text_embeds != [] else None
